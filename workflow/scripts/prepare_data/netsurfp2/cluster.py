"""Cluster NetSurfP-2.0 training sequences using MMseqs2 easy-cluster.

Reads ``train_full.parquet``, writes sequences to a temporary FASTA file,
runs ``mmseqs easy-cluster`` at the configured sequence identity threshold,
and writes the cluster assignments to a TSV file.
...
logger.info(
    "Clustering NetSurfP-2.0 training sequences at %.0f%% identity",
    min_seq_id * 100,
)

df = pd.read_parquet(train_full_parquet)
sequences = df["sequence"].tolist()
logger.info("Read %d training sequences", len(sequences))
...
logger.info(
    "NetSurfP-2.0: %d sequences -> %d clusters (%.1f%% reduction)",
    len(sequences),
    n_clusters,
