...
def test_train_multitask_model_accumulate_across_tasks(monkeypatch):
    record.clear()
    ...
    train_module.train_multitask_accgrad_lora(
        model=model,
        tokenizer=tokenizer,
    ...
    assert scheduler.step_calls == 4
    assert scheduler.total_steps == 4


def test_train_multitask_model_resume_mid_epoch_from_validation_checkpoint_is_deterministic_accumulate_across_tasks(
    tmp_path,
    monkeypatch,
):
    hidden_size = 4
    ...
    logger = logging.getLogger(
        "test_resume_mid_epoch_deterministic_accumulate_across_tasks"
    )
...
def test_train_multitask_accgrad_history_has_validation_step_metadata(monkeypatch):
    record.clear()
    monkeypatch.setattr(train_module.torch.optim, "AdamW", DummyOptimizer)
    monkeypatch.setattr(
        train_module,
    ...
    _, history, _ = train_module.train_multitask_accgrad_lora(
        model=model,
        tokenizer=object(),
        checkpoint="checkpoint",
        task_datasets=task_datasets,
...
def test_train_multitask_accgrad_runs_final_validation_when_steps_not_multiple(
    monkeypatch,
):
    record.clear()
    monkeypatch.setattr(train_module.torch.optim, "AdamW", DummyOptimizer)
...
def test_train_multitask_accgrad_checkpoint_hash_mismatch_starts_fresh(
    tmp_path, caplog
):
    hidden_size = 4
    num_labels = 2
    ...
    with caplog.at_level(logging.WARNING):
        _, history_resumed, _ = train_module.train_multitask_accgrad_lora(
            model=model_resumed,
            tokenizer=None,
            checkpoint="facebook/esm2_t6_8M_UR50D",
            task_datasets=task_datasets,
...
def test_train_multitask_accgrad_logs_named_metrics(monkeypatch):
    record.clear()
    monkeypatch.setattr(train_module.torch.optim, "AdamW", DummyOptimizer)
    monkeypatch.setattr(
        train_module,
...
def test_train_multitask_accgrad_validate_every_n_train_batches(monkeypatch):
    record.clear()
    monkeypatch.setattr(train_module.torch.optim, "AdamW", DummyOptimizer)
    monkeypatch.setattr(
        train_module,
...
def test_train_multitask_accgrad_early_stopping(monkeypatch):
    record.clear()
    monkeypatch.setattr(train_module.torch.optim, "AdamW", DummyOptimizer)
    monkeypatch.setattr(
        train_module,
...
def test_train_multitask_accgrad_early_stopping_custom_metric(monkeypatch):
    record.clear()
    monkeypatch.setattr(train_module.torch.optim, "AdamW", DummyOptimizer)
    monkeypatch.setattr(
        train_module,
...
def test_train_multitask_accgrad_checkpointing(tmp_path):
    hidden_size = 4
    num_labels = 2
    dropout = 0.0
    seq_len = 3
...
def test_train_multitask_accgrad_resume_preserves_shuffled_order_epoch_boundary(
    tmp_path,
    monkeypatch,
):
    hidden_size = 4
...
def test_train_multitask_accgrad_checkpoint_resume_continues_correctly(tmp_path):
    hidden_size = 4
    num_labels = 2
    dropout = 0.0
    seq_len = 3
...
def test_train_multitask_accgrad_checkpoint_resume_matches_uninterrupted(
    ...
    _, history_resumed, best_resumed = train_module.train_multitask_accgrad_lora(
        model=create_model(),
        tokenizer=None,
        checkpoint="facebook/esm2_t6_8M_UR50D",
        task_datasets=create_datasets(),
...
def test_train_multitask_accgrad_best_so_far_tracking_and_saving(tmp_path, monkeypatch):
    ...
    _, history, best_checkpoints = train_module.train_multitask_accgrad_lora(
        model=model,
        tokenizer=object(),
        checkpoint="checkpoint",
        task_datasets=task_datasets,
...
def test_train_multitask_accgrad_resume_with_completed_epochs_runs_final_validation(
    tmp_path,
):
    ...
    def create_datasets():
        return {
            "task_a": DummyTaskDataset("task_a", num_samples, seq_len, hidden_size),
            "task_b": DummyTaskDataset("task_b", num_samples, seq_len, hidden_size),
        }

    checkpoint_dir = tmp_path / "checkpoints"
    logger = logging.getLogger("test_resume_completed_epochs_accgrad")
