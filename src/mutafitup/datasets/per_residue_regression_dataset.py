...
class PerResidueRegressionDataset(BaseMultitaskDataset):
    def __init__(
        self,
        name: str,
        train_parquet: str,
    ...
    def _normalize_and_mask(self, train_df, valid_df):
        all_train_scores = []
        for scores in train_df[self.label_column]:
            for s in scores:
                if s != self.unresolved_marker:
                    all_train_scores.append(s)

        if not all_train_scores:
            raise ValueError("No resolved scores found for normalization")

        label_min = float(np.min(all_train_scores))
        label_max = float(np.max(all_train_scores))
        scale_factor = max(abs(label_min), abs(label_max)) or 1.0

        def normalize_row(values):
            return [
                v if v == self.unresolved_marker else v / scale_factor for v in values
            ]

        def mask_unresolved(values):
            return [-100.0 if v == self.unresolved_marker else v for v in values]

        train_df = train_df.copy()
        valid_df = valid_df.copy()

        train_df[self.label_column] = train_df[self.label_column].apply(normalize_row)
        valid_df[self.label_column] = valid_df[self.label_column].apply(normalize_row)
    ...
    def _normalize_df(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df = df.copy()
        df[self.label_column] = df[self.label_column].apply(normalize_row)
