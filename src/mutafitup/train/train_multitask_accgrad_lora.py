"""Training module for accumulated gradient LoRA-based multitask model training.

This approach uses Low-Rank Adaptation (LoRA) with gradient accumulation across
all tasks before performing an optimizer step.
"""
...
from .strategy import AccGradLoRAStrategy


def train_multitask_accgrad_lora(
    ...
    """Train a multitask model using LoRA with accumulated gradients.

    This function trains the model using Low-Rank Adaptation (LoRA) with
    gradient accumulation across all tasks. Each task's loss is backpropagated
    immediately for memory efficiency, and gradients accumulate additively
    ...
    """
    strategy = AccGradLoRAStrategy(uncertainty_weighting=uncertainty_weighting)
