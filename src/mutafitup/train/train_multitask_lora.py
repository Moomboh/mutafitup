"""Training module for LoRA-based multitask model training.

This approach uses Low-Rank Adaptation (LoRA) to fine-tune the backbone
while training task-specific heads.
"""
...
from .strategy import LoRAStrategy


def train_multitask_lora(
    ...
    This function trains the model using Low-Rank Adaptation (LoRA) to
    fine-tune the backbone along with task-specific heads. Each task is
    trained separately without gradient accumulation.
    ...
    trainer = MultitaskTrainer(
        ...
        seed=seed,
        logger=logger,
        embedding_cache_dir=None,  # LoRA cannot use embedding cache
